...
def compute_tv_distance(samples, pdf_fn, limits, bins=100):
    ...
    # Calculate direct PDF values
    true_pdf = np.array([pdf_fn(p) for p in grid_points]).reshape(bins, bins)
    
    # Avoid division by zero if true_pdf is entirely zero (underflow)
...
def run_isir(target_fn, proposal, x0, num_samples, N=10):
    ...
    for t in range(1, num_samples):
        ...
        pi_Y = np.array([target_fn(y) for y in Y])
        q_Y = proposal.pdf(Y)
...
def get_banana_potential_jax(nu):
    def potential(z):
        ...
        return -jnp.log(prob + 1e-12) # +1e-12 prevents log(0) crash
    return potential
...
def get_gmm_potential_jax(weights, means, covs):
    ...
    def potential(z):
        ...
        for k in range(len(w)):
            # Sum linear probabilities directly without jax_logsumexp
...
def plot_results(samples_dict, target_density_fn, true_samples, title, limits):
    ...
    for i in range(100):
        for j in range(100):
            Z[i, j] = target_density_fn([X[i, j], Y[i, j]])
            
    fig, axes = plt.subplots(1, 4, figsize=(20, 5))
    fig.suptitle(title, fontsize=16)
    
    for ax, (name, samples) in tqdm(zip(axes, samples_dict.items())):
        ...
        ax.scatter(samples[::5, 0], samples[::5, 1], s=5, alpha=0.3, color='red')
...
if __name__ == "__main__":
    ...
    for nu in tqdm([2.0, 5.0, 12.0, 20.0]):
        ...
        target_fn = lambda x: pi_banana(x, nu=nu) # Using linear pi
    ...
    cov_base1 = np.array([[1, 0.5], [0.5, 1]])
    cov_base2 = np.array([[0.4, -0.5], [-0.5, 0.6]])
    cov_base3 = np.array([[1, 0.75], [0.75, 1]])
    configs = {
    ...
    for name, config in tqdm(configs.items()):
        ...
        target_fn = lambda x: pi_gmm(x, **config) # Using linear pi
